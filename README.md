...
1. When using Git locally, what are these?  Define each one in a sentence
   * Staging area - the working directory showing what've you done to your files
   * Working copy - cloned files that working and can write codes on
...
3. There are 2 ways to create a local Git repository.  Briefly describe each one:
   - create from local directory by using command git init in the terminal where your directory is
   - from existing repository in github , copy that repo link and use git clone <link> command in terminal 
...
**TODO**: This section is free-form.  Create 4 numbered items for common branch-and-merge tasks you would like to remember and show the git command to do each one. (You are write *more* than 4 if you want.)
   ...
   ```
   ```
   "Merging the changes from branch before into the current branch"
   git merge newbranch  
   ```
   ```
   "Deleting a branch after it has been merged"  
   git branch -d newbranch
   ```
   ```
   "Forcefully deleting a branch (use with caution)"
   git branch -D newbranch 
   ```
...
> git status -> check current status , git add -u -> add modified files
